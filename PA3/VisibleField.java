...
public class VisibleField {
   ...
   public VisibleField(MineField mineField) {
      ...
      totalrow=mineField.numRows();
      totalcol=mineField.numCols();
      status = new int [totalrow][totalcol];
      NofM=mineField.numMines();
   ...
   */
   ...
   public int getStatus(int row, int col) {
      assert getMineField().inRange(row,col):"row or col not in range";
   ...
   public boolean uncover(int row, int col) {
      ...
      {
         status[row][col]=EXPLODED_MINE;//turn to red
         return false;
      }
      else {
         continuing(row,col);
   ...
   public boolean isGameOver() {
      ...
      for(int i =0; i<totalrow; i++)
      {
         for (int j=0; j<totalcol; j++)
         {
            ...
            else if (status[i][j]>=UNCOVERED){
               count ++;
            }
         }
      }

      if (count == totalcol*totalrow-NofM)
      {
         win();
         return true;
      }
   ...
   public boolean isUncovered(int row, int col) {
     assert getMineField().inRange(row,col):"row or col not in range";
     if (getStatus(row,col)>=UNCOVERED)
     {
        return true;
     }
     else{
   ...
   //continue uncovering
   private void continuing (int row, int col)
   {
      if (getMineField().numAdjacentMines(row,col)==0)
      {
         status[row][col]=UNCOVERED;
         for (int i=row-1; i<row+2; i++)
         {
            for (int j=col-1; j<col+2;j++)
            {
               if (getMineField().inRange(i,j)==true)
               {
                  if (!(status[i][j]==MINE_GUESS||status[i][j]==QUESTION)
                          &&!(i==row&&j==col)&&status[i][j]==COVERED)
                  {
                     continuing(i,j);
                  }
               }
   ...
   private void lose ()
   {
      for(int i =0; i<totalrow; i++)
      {
         for (int j = 0; j < totalcol; j++)
         {
            if (mineField.hasMine(i,j)==false&&status[i][j]==MINE_GUESS)
            {
               status[i][j]=INCORRECT_GUESS;//turn to X
            }
            else if (mineField.hasMine(i,j)==true&&status[i][j]!=EXPLODED_MINE&&status[i][j]!=MINE_GUESS)
            {
               status[i][j]=MINE;//turn to black
            }
         }
      }
   }
   ...
   private void win ()
   {
      for(int i =0; i<totalrow; i++)
      {
         for (int j=0; j<totalcol; j++)
         {
            ...
            {
               status[i][j]=MINE_GUESS;//turn to yellow
            }
         }
      }
   }
